...
int main(int argc, char** argv) {
    ...
    // Allocation mémoire pour le bloc de lignes de A et le vecteur X
    int* block_A = (int*)malloc(block_size * MATRIX_SIZE * sizeof(int));
    int* block_X = (int*)malloc(block_size * sizeof(int));

    // Initialisation du bloc de lignes de A et du vecteur X
    for (int i = 0; i < block_size; i++) {
        for (int j = 0; j < MATRIX_SIZE; j++) {
            block_A[i * MATRIX_SIZE + j] = (block_offset + i + 1) * (j + 1);
        }
        block_X[i] = block_offset + i + 1;
    }

    // Collecte de tous les blocs de lignes de A
    ...
    // Diffusion du vecteur X à tous les processeurs
    ...
    for (int i = 0; i < block_size; i++) {
        local_result[i] = 0;
        for (int j = 0; j < MATRIX_SIZE; j++) {
            local_result[i] += all_A[(block_offset + i) * MATRIX_SIZE + j] * all_X[j];
        }
    }
